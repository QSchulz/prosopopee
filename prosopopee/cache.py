...
class Cache:
    ...
    def needs_to_be_generated(self, source, target, options):
        if not os.path.exists(target):
            logging.debug("%s does not exist. Requesting generation...", target)
            return True

        if target not in self.cache:
            logging.debug("%s not in cache. Requesting generation...", target)
            return True

        cached_picture = self.cache[target]

        if cached_picture["size"] != os.path.getsize(source):
            logging.debug("%s has different size than in cache. Requesting generation...",
                    target)
            return True

        options = remove_superficial_options(options)
        # json.dump() transforms tuples into list, so to be able to compare options
        ...
        if cached_picture["options"] != options:
            logging.debug("%s has different options than in cache."
                    " Requesting generation...", target)
            return True

        logging.debug("(%s) Skipping cached thumbnail %s", source, target)
        return False
